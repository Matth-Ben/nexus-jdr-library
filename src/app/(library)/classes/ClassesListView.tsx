...
export function ClassesListView({ classes, totalCount, query, loadError,
  openId,
          ...
          <ul className={styles.list}>
            {classes.map((klass) => (
              <li key={klass.id} className={styles.row}>
                <Link
                  href={panelHref("/classes", { q: query }, klass.id)}
                  scroll={false}
                  aria-current={openId === klass.id ? "true" : undefined}
                  className={`${styles.rowLink} ${openId === klass.id ? styles.rowLinkActive : ""}`}
                >
